...
/**
 * This lambda verify the authorization and only if the token is valid
 * the target lambda (e.g., createAuction) is executed
 */
export async function handler(event, context) {
  ...
  try {
    const claims = jwt.verify(token, process.env.AUTH0_PRIVATE_KEY);
